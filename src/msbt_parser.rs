use std::{fs::File, io::{self, Read}, path::Path, str::Utf8Error};
...
impl MSBTParser {
    fn new(data: Vec<u8>) -> Self {
        ...
        let parsed = MSBTParser::parse_data(&data).unwrap_or(MSBTData { header: Default::default(), blocks: Vec::new() });
    ...
    fn parse_data(data: &[u8]) -> Result<MSBTData, MSBTParseError>{
        ...
        let header = MSBTParser::parse_header(data)?;
         ...
         Ok(MSBTData { header, blocks: Vec::new() })
    }
